...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        @if(isset($isCollectionContext) && $isCollectionContext)
            <a href="{{ route('admin.collections.show', $collectionName) }}" 
        ...
        @else
            ...
            <a href="{{ $backRoute }}" 
                ...
                <span class="tracking-tight uppercase text-xs">Collection Overview</span>
            </a>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-[2.5rem] p-6 md:p-12 shadow-2xl relative overflow-hidden border border-slate-100">


                {{-- Category Title & Admin Actions --}}
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10 relative z-10">
                    <span class="inline-flex items-center gap-2 bg-lib-light backdrop-blur-md px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-[0.25em] border border-lib-sky/20 text-lib-navy">
                    ...
                    <div class="flex gap-3">
                        <a href="{{ route('admin.assets.edit', $asset->id) }}" 
                           class="inline-flex items-center gap-2.5 bg-lib-navy hover:bg-lib-sky text-white px-5 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl">
                        ...
                        <form action="{{ route('admin.assets.destroy', $asset->id) }}" method="POST" class="inline">
                            @csrf 
                            @method('DELETE')
                            <button type="submit" class="delete-confirm inline-flex items-center gap-2.5 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white px-5 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all border border-red-500/20 shadow-xl backdrop-blur-md" 
                ...
                <div id="fullscreenContainer" class="relative">
                    ...
                    <div class="media-inner-box bg-slate-50 backdrop-blur-sm rounded-[2rem] p-4 flex items-center justify-center min-h-[500px] relative border border-slate-200 shadow-inner transition-all duration-500 overflow-hidden">
                        ...
                        <img id="mainImage"
                             ...
                             class="max-w-full max-h-[460px] object-contain transition-all duration-700 ease-in-out transform scale-100 hover:scale-[1.02] {{ $asset->type === 'photo' ? 'block' : 'hidden' }}">

                        {{-- Full Screen Button (Photos and E-books) --}}
                        <button id="fullscreenBtn" onclick="toggleFullScreen()"
                                class="absolute top-6 right-6 bg-lib-navy/80 hover:bg-lib-navy text-white border border-lib-navy rounded-2xl p-4 transition-all z-50 backdrop-blur-md shadow-2xl group/fs {{ in_array(strtolower($asset->type), ['photo','ebook']) ? 'block' : 'hidden' }}"
                        ...
                        {{-- Video Element --}}
                        <video id="mainVideo"
                               src="{{ $asset->type === 'video' ? asset('storage/' . $asset->file_path) : '' }}"
                               controls
                               class="max-w-full max-h-[460px] {{ $asset->type === 'video' ? 'block' : 'hidden' }}">
                        </video>

                        {{-- E-book (PDF) Element --}}
                        <div id="mainEbook" class="w-full {{ $asset->type === 'ebook' ? 'block' : 'hidden' }}">
                            <div class="relative w-full h-[700px]">
                                <iframe id="ebookFrame" 
                                        src="{{ $asset->type === 'ebook' ? asset('storage/' . $asset->file_path) . '#toolbar=1&navpanes=1&scrollbar=1&view=FitH' : '' }}"
                                        class="w-full h-full border-none rounded-lg shadow-inner"
                                        allow="fullscreen">
                                        <p class="text-center text-slate-500 p-8">Your browser does not support PDFs. <a href="{{ $asset->type === 'ebook' ? asset('storage/' . $asset->file_path) : '' }}" class="text-lib-sky hover:text-lib-navy font-bold">Download the PDF</a>.</p>
                                </iframe>
                            </div>
                            <div class="flex gap-3 justify-center mt-4">
                                <a id="ebookLink" href="{{ $asset->type === 'ebook' ? asset('storage/' . $asset->file_path) : '' }}" 
                                   target="_blank" 
                                   download
                                ...
                                <a id="ebookOpenLink" href="{{ $asset->type === 'ebook' ? asset('storage/' . $asset->file_path) : '' }}" 
                                   target="_blank"
                ...
                <div class="mt-10 mb-6 relative z-10 flex flex-col justify-center">
                    <p id="mainDate" class="text-lib-sky text-[10px] font-black uppercase tracking-[0.3em] mb-3 slide-up">
                        {{ \Carbon\Carbon::parse($asset->date ?? $asset->created_at)->format('d F Y') }}
                    </p>
                    <h3 id="mainTitle" class="text-3xl md:text-4xl font-black tracking-tight leading-tight text-lib-navy slide-up-delayed">
                        {{ $asset->title }}
                    </h3>
                </div>

                {{-- METADATA SECTION (Moved from right sidebar) --}}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-grow">
                                ...
                                <a href="{{ route('admin.collections.show', $collectionName) }}" class="text-sm font-bold text-lib-sky hover:text-lib-navy transition-colors">
                                    {{ $collectionName }}
                                </a>
                            </div>
                        </div>
