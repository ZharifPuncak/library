...
@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ filterModalOpen: {{ request('filters') === 'open' ? 'true' : 'false' }}, addModalOpen: false }">
    ...
    <div class="mb-10 flex flex-col gap-6">
        <div>
            <h1 class="text-4xl font-black text-lib-navy uppercase tracking-tighter mb-2">Photos</h1>
            <p class="text-slate-500 font-medium text-lg">Browse and manage all photo assets in the corporate repository.</p>
        </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-12">
        <div class="flex-grow relative group">
            <form action="{{ route('admin.assets.photos') }}" method="GET" class="relative">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.assets.photos', request()->except('search')) }}" 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 auto-rows-fr">
                ...
                <div class="p-4 flex-grow flex flex-col">
                    <div class="flex-grow">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[9px] font-black text-lib-sky uppercase tracking-[0.2em]">{{ $asset->categories->first()->name ?? 'Photo' }}</span>
                        </div>
        ...
        @empty
            <div class="col-span-full py-20 text-center">
                ...
                <a href="{{ route('admin.assets.photos') }}" class="inline-block mt-8 px-10 py-4 bg-lib-blue text-white rounded-2xl font-black hover:bg-lib-navy transition-all shadow-xl shadow-lib-blue/20">Reset Photos</a>
            </div>
        @endforelse
    </div>
        ...
        <div x-show="filterModalOpen" x-cloak 
            ...
            <div @click.away="filterModalOpen = false" 
                ...
                <div class="mb-10 text-center sm:text-left">
                    ...
                    <p class="text-slate-400 font-medium mt-1">Filter photos by category and year</p>
                </div>

                <form action="{{ route('admin.assets.photos') }}" method="GET" class="space-y-10">
                    ...
                    <div class="flex gap-4 pt-4">
                        ...
                        @if(request()->hasAny(['categories', 'year']))
                            <a href="{{ route('admin.assets.photos', request()->only('search')) }}" 
    ...
    <template x-teleport="body">
        <div x-show="addModalOpen" x-cloak 
            ...
            <div @click.away="addModalOpen = false" 
                 class="bg-white w-full sm:max-w-2xl rounded-t-[3.5rem] sm:rounded-[4rem] p-10 pb-12 shadow-2xl relative max-h-[90vh] overflow-y-auto"
                 x-transition:enter="transition ease-out duration-500"
                 x-transition:enter-start="translate-y-full sm:scale-90 sm:translate-y-0"
                 x-transition:enter-end="translate-y-0 sm:scale-100 sm:translate-y-0">
                ...
                <div class="mb-10 text-center sm:text-left">
                    <h2 class="text-3xl font-black text-lib-navy tracking-tighter uppercase">Add New Photo</h2>
                    <p class="text-slate-400 font-medium mt-1 italic">Contribute to the corporate memory bank</p>
                </div>
                ...
                <form action="{{ route('admin.assets.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <h3 class="text-xs font-black text-slate-300 uppercase tracking-widest mb-4 italic">The Content</h3>
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Choose File *</label>
                                    <input type="file" name="file" accept="image/*" required
                                           class="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl px-6 py-10 text-center cursor-pointer hover:border-lib-sky transition-all font-bold text-slate-400">
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Photo Name *</label>
                                    <input type="text" name="title" placeholder="e.g. Annual General Meeting 2025" required
                    ...
                    <div class="flex gap-4 pt-6">
                        <button type="submit" class="flex-1 bg-lib-blue text-white py-5 rounded-3xl font-black hover:bg-lib-navy transition-all shadow-2xl shadow-lib-blue/30 text-lg uppercase tracking-widest hover:-translate-y-1">Upload Photo</button>
                    </div>
                </form>
            </div>
        </div>
