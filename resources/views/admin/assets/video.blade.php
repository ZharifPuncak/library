...
@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ filterModalOpen: false, addModalOpen: false }">
    ...
    <div class="mb-10 flex flex-col gap-6">
        <div>
            <h1 class="text-4xl font-black text-lib-navy uppercase tracking-tighter mb-2">Videos</h1>
            <p class="text-slate-500 font-medium text-lg">Manage video content from corporate events, training sessions, and archival footage.</p>
        </div>
    ...
    {{-- Search Bar with Filter Button (matching all media) --}}
    <div class="flex flex-col md:flex-row gap-4 mb-12">
        <div class="flex-grow relative group">
            <form action="{{ route('admin.assets.videos') }}" method="GET" class="relative">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.assets.videos', request()->except('search')) }}" 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 auto-rows-fr">
        @forelse($assets as $asset)
            <div class="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col font-fredoka"
                 onmouseenter="this.querySelector('video')?.play(); this.querySelector('.action-buttons').style.opacity='1'; this.querySelector('.action-buttons').style.transform='translateY(0)';" 
                 onmouseleave="this.querySelector('video')?.load(); this.querySelector('.action-buttons').style.opacity='0'; this.querySelector('.action-buttons').style.transform='translateY(10px)';">
                ...
                <div class="p-4 flex-grow flex flex-col">
                    <div class="flex-grow">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[9px] font-black text-lib-sky uppercase tracking-[0.2em]">{{ $asset->categories->first()->name ?? 'Video' }}</span>
                        </div>
        ...
        @empty
            <div class="col-span-full py-20 text-center">
                ...
                <a href="{{ route('admin.assets.videos') }}" class="inline-block mt-8 px-10 py-4 bg-lib-blue text-white rounded-2xl font-black hover:bg-lib-navy transition-all shadow-xl shadow-lib-blue/20">Reset Videos</a>
            </div>
        @endforelse
    </div>
    ...
    <template x-teleport="body">
        <div x-show="filterModalOpen" x-cloak 
            ...
            <div @click.away="filterModalOpen = false" 
                ...
                <form action="{{ route('admin.assets.videos') }}" method="GET" class="space-y-10">
                    ...
                    <div class="flex gap-4 pt-4">
                        ...
                        @if(request()->hasAny(['categories', 'year']))
                            <a href="{{ route('admin.assets.videos', request()->only('search')) }}" 
    ...
    <template x-teleport="body">
        <div x-show="addModalOpen" x-cloak 
            ...
            <div @click.away="addModalOpen = false" 
                 class="bg-white w-full sm:max-w-3xl rounded-t-[3.5rem] sm:rounded-[4rem] p-10 pb-12 shadow-2xl relative max-h-[90vh] overflow-y-auto"
                 x-transition:enter="transition ease-out duration-500"
                 x-transition:enter-start="translate-y-full sm:scale-90 sm:translate-y-0"
                 x-transition:enter-end="translate-y-0 sm:scale-100 sm:translate-y-0">
                
                <div @click="addModalOpen = false" class="absolute right-8 top-8 p-3 bg-slate-50 text-slate-400 rounded-full hover:bg-red-50 hover:text-red-500 cursor-pointer transition-all">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"/></svg>
                </div>

                <div class="mb-10 text-center sm:text-left">
                    <h2 class="text-3xl font-black text-lib-navy tracking-tighter uppercase">Add New Video</h2>
                    <p class="text-slate-400 font-medium mt-1 italic">Archive moments that define our progress</p>
                </div>
                ...
                <form action="{{ route('admin.assets.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <h3 class="text-xs font-black text-slate-300 uppercase tracking-widest mb-4 italic">The Media</h3>
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Upload Video *</label>
                                    <input type="file" name="file" accept="video/*" required
                                           class="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl px-6 py-8 text-center cursor-pointer hover:border-lib-sky transition-all font-bold text-slate-400 text-xs">
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 ml-1 italic">Max size: 500MB</p>
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Thumbnail (Optional)</label>
                                    <input type="file" name="thumbnail" accept="image/*"
                                           class="w-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl px-6 py-6 text-center cursor-pointer hover:border-lib-sky transition-all font-bold text-slate-400 text-xs">
                                </div>
                            </div>
                        </div>

                        <div>
                            <h3 class="text-xs font-black text-slate-300 uppercase tracking-widest mb-4 italic">The Context</h3>
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Video Name *</label>
                                    <input type="text" name="title" placeholder="e.g. Employee Townhall Q1" required
                                           class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-lib-sky transition-all font-bold text-lib-navy shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Event Date *</label>
                                    <input type="date" name="date" value="{{ date('Y-m-d') }}" required
                                           class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-lib-sky transition-all font-bold text-lib-navy shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Category *</label>
                                    <select name="category_id" required
                                            class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-lib-sky transition-all font-bold text-lib-navy shadow-sm appearance-none">
                    ...
                    <div>
                        <h3 class="text-xs font-black text-slate-300 uppercase tracking-widest mb-4 italic ml-1">Indexing & Tags</h3>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                @foreach($tags ?? [] as $tag)
                                    <label class="cursor-pointer group">
                                        ...
                                        <div class="px-4 py-2.5 text-center rounded-xl border-2 border-white bg-white text-[10px] font-black uppercase text-slate-400 peer-checked:bg-lib-sky peer-checked:border-lib-sky peer-checked:text-white transition-all group-hover:border-slate-200 shadow-sm">
                                            #{{ $tag->name }}
                                        </div>
                                    </label>
                                @endforeach
                            </div>
                            <p class="text-center text-[10px] text-slate-300 font-bold uppercase tracking-widest mt-6 italic">Select relevant tags for better searchability</p>
                        </div>
                    </div>

                    <div class="flex gap-4 pt-6">
                        <button type="submit" class="flex-1 bg-lib-blue text-white py-5 rounded-3xl font-black hover:bg-lib-navy transition-all shadow-2xl shadow-lib-blue/30 text-lg uppercase tracking-widest hover:-translate-y-1">Upload Video</button>
                    </div>
                </form>
            </div>
        </div>
