...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ filterModalOpen: false, addModalOpen: false }">
    ...
    <div class="mb-10 flex flex-col gap-6">
        <div>
            <h1 class="text-4xl font-black text-lib-navy uppercase tracking-tighter mb-2">e-books</h1>
            <p class="text-slate-500 font-medium text-lg">Access and manage the digital library collection. Browse publications, reports, and educational materials.</p>
        </div>
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-12">
        <div class="flex-grow relative group">
            <form action="{{ route('admin.assets.ebooks') }}" method="GET" class="relative">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.assets.ebooks', request()->except('search')) }}" 
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 auto-rows-fr">
                        ...
                        @else
                            ...
                            <div class="w-full h-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center">
                                <div class="text-center text-white">
                                    ...
                                    <div class="text-xs opacity-80 font-bold">PDF</div>
                                </div>
                            </div>
                        @endif
                ...
                <div class="p-4 flex-grow flex flex-col">
                    <div class="flex-grow">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-[9px] font-black text-lib-sky uppercase tracking-[0.2em]">{{ $asset->categories->first()->name ?? 'e-book' }}</span>
                        </div>
        ...
        @empty
            <div class="col-span-full py-20 text-center">
                ...
                <a href="{{ route('admin.assets.ebooks') }}" class="inline-block mt-8 px-10 py-4 bg-lib-blue text-white rounded-2xl font-black hover:bg-lib-navy transition-all shadow-xl shadow-lib-blue/20">Reset e-books</a>
            </div>
        @endforelse
    </div>
    ...
    <template x-teleport="body">
        <div x-show="filterModalOpen" x-cloak 
            ...
            <div @click.away="filterModalOpen = false" 
                ...
                <form action="{{ route('admin.assets.ebooks') }}" method="GET" class="space-y-10">
                    ...
                    <div class="flex gap-4 pt-4">
                        ...
                        @if(request()->hasAny(['categories', 'year']))
                            <a href="{{ route('admin.assets.ebooks', request()->only('search')) }}" 
    ...
    <template x-teleport="body">
        <div x-show="addModalOpen" x-cloak 
            ...
            <div @click.away="addModalOpen = false" 
                ...
                <div class="mb-10 text-center sm:text-left">
                    <h2 class="text-3xl font-black text-lib-navy tracking-tighter uppercase">Add New e-book</h2>
                    <p class="text-slate-400 font-medium mt-1 italic">Preserve corporate knowledge and reports</p>
                </div>
                ...
                <form action="{{ route('admin.assets.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <h3 class="text-xs font-black text-slate-300 uppercase tracking-widest mb-4 italic">The Publication</h3>
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Upload PDF *</label>
                                    ...
                                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 ml-1 italic">Max size: 50MB</p>
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Cover Image (Optional)</label>
                                ...
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">ISBN (Optional)</label>
                        ...
                        <div>
                            ...
                            <div class="space-y-6">
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">e-book Name *</label>
                                    <input type="text" name="title" placeholder="e.g. Annual Sustainability Report 2024" required
                                           class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 focus:ring-2 focus:ring-lib-sky transition-all font-bold text-lib-navy shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-lib-navy uppercase mb-2 ml-1">Publication Date *</label>
                    ...
                    <div class="flex gap-4 pt-6">
                        <button type="submit" class="flex-1 bg-lib-blue text-white py-5 rounded-3xl font-black hover:bg-lib-navy transition-all shadow-2xl shadow-lib-blue/30 text-lg uppercase tracking-widest hover:-translate-y-1">Upload e-book</button>
                    </div>
                </form>
            </div>
        </div>
