...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ editModalOpen: false, editName: '', editId: null }">
    ...
    {{-- Header --}}
    <div class="mb-10">
        <h1 class="text-4xl font-black text-lib-navy uppercase tracking-tighter mb-2">Manage Tags</h1>
        <p class="text-slate-500 font-medium text-lg">Label your assets with descriptive tags for easier discovery.</p>
    </div>
    ...
    <div class="bg-white p-8 rounded-[2.5rem] shadow-xl border border-slate-100 mb-10 relative overflow-hidden">
        ...
        <form action="{{ route('admin.tags.store') }}" method="POST" class="relative z-10">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                <div class="md:col-span-10">
                    <label for="tag_name" class="block font-bold text-slate-400 text-xs uppercase tracking-wider mb-2 ml-1">Tag Name <span class="text-red-400">*</span></label>
                    <input type="text" id="tag_name" name="name" required placeholder="e.g. Graduation, Competition, Award"
    ...
    <div class="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden">
        <div class="p-8 border-b border-slate-100">
            <h2 class="text-xl font-black text-lib-navy flex items-center gap-2 uppercase tracking-wide">
                <span>🏷️</span> Existing Tags <span class="bg-lib-light text-lib-sky px-3 py-1 rounded-full text-xs ml-2">{{ count($tags) ?? 0 }}</span>
            </h2>
        </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-8 py-4 text-left font-bold text-slate-400 text-xs uppercase tracking-wider">Tag Name</th>
                            <th class="px-8 py-4 text-left font-bold text-slate-400 text-xs uppercase tracking-wider">Assets Count</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($tags as $tag)
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                <td class="px-8 py-5 text-slate-500 font-bold font-mono text-sm">#{{ $tag->id }}</td>
                                <td class="px-8 py-5 text-lib-navy font-black text-lg">{{ $tag->name }}</td>
                                <td class="px-8 py-5 text-slate-500 font-bold text-sm">{{ $tag->assets_count ?? 0 }} assets</td>
                                <td class="px-8 py-5 text-slate-400 font-medium text-sm">{{ $tag->created_at->format('d M Y') }}</td>
                                ...
                                <td class="px-8 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-100 translate-y-0 lg:opacity-0 lg:translate-y-2 lg:group-hover:opacity-100 lg:group-hover:translate-y-0 transition-all duration-300">
                                        ...
                                        {{-- Edit Button --}}
                                        <button @click="editId = {{ $tag->id }}; editName = '{{ addslashes($tag->name) }}'; editModalOpen = true" 
                                        ...
                                        {{-- Delete Form --}}
                                        <form action="{{ route('admin.tags.destroy', $tag->id) }}" method="POST" class="inline-block" onsubmit="return confirmDelete(event, '{{ $tag->name }}')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
        ...
        @else
            <div class="text-center py-12">
                <div class="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center text-3xl mx-auto mb-4 text-slate-300">🏷️</div>
                <h3 class="text-lg font-bold text-slate-600 mb-1">No Tags Found</h3>
                <p class="text-slate-400 text-sm">Add your first tag above.</p>
            </div>
        @endif
    </div>
    ...
    <div x-show="editModalOpen" x-cloak 
        ...
        <div class="bg-white w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl transform transition-all"
            ...
            <h3 class="text-2xl font-black text-lib-navy mb-6 uppercase tracking-tight">Edit Tag</h3>
            
            <form :action="`{{ url('admin/tags') }}/${editId}`" method="POST">
                @csrf
                @method('PUT')
                
                <div class="space-y-6">
                    <div>
                        <label class="block font-bold text-slate-400 text-xs uppercase tracking-wider mb-2 ml-1">Tag Name</label>
    ...
    <script>
        ...
        function confirmDelete(event, name) {
            ...
            Swal.fire({
                title: 'Delete Tag?',
                text: `Are you sure you want to delete this tag? This might affect existing assets.`,
                icon: 'warning',
