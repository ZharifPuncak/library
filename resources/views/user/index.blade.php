    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="flex flex-col gap-4 p-6 border-b border-slate-100">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <h1 class="text-xl font-bold text-lib-navy">Users</h1>
                    <p class="text-sm text-slate-500">Manage accounts that can access the library.</p>
                </div>
        ...
        @if(!empty($search))
            <div class="px-6 py-3 bg-lib-light/40 border-b border-slate-100 text-xs text-slate-600">
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 text-slate-500 text-[11px] font-bold uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="text-left px-6 py-3 hidden md:table-cell">Email</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($users as $user)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-3 text-slate-500 hidden md:table-cell">{{ $user->email }}</td>
                            <td class="px-6 py-3">
                                @if($user->isAdmin())
                            ...
                            <td class="px-6 py-3 text-right">
                                <div class="inline-flex items-center gap-1">
                                    <a href="{{ route('users.edit', $user) }}"
                                       class="px-3 py-1.5 rounded-lg text-xs font-bold text-slate-600 hover:bg-slate-100 hover:text-lib-navy transition-colors">Edit</a>
                                    @if($user->id !== auth()->id())
